...
def migrate_job():
    ...
    # cursor.execute("""
    #     select id,url,status,server_job.desc,platform,source,crawl_type,regulation,charset,interval,content_regulation from server_job;
    #     """)
...
def migrate_document():
    # cursor = connection.cursor()
    # cursor.execute('select job_id,title,url,url_hash,content from server_document')
    # documents = []
    # items = cursor.fetchall()
    # for item in items:
    #     documents.append({
    ...
    for job in jobs:
        job_id = Job.objects.get(pk=job.get('job_id'))
        del job['job_id']
        j = Document(**job)
        j.job_id = job_id
        j.save()
